...
- **Recency is enforced explicitly, not left to RSS ranking.** Google News RSS itself has no `after:`/`before:` date operator, so a dedicated recency filter runs immediately after ingestion (`run_pipeline.py`), parsing each article's real `pubDate` and dropping anything older than a configurable lookback window (default 90 days, adjustable via a slider in the dashboard from 7–365 days) before Stage 1 even runs. Articles with an unparseable or missing date are dropped rather than assumed recent, since a "real-time" claim shouldn't rest on an unverifiable date. The newsletter header also displays the actual covered date range, computed directly from the surviving articles' dates (not LLM-guessed), so the claimed window is always accurate.
- **"Real-time" here means on-demand, not scheduled.** The pipeline re-fetches live news and regenerates the newsletter whenever a user clicks "Generate Newsletter" — there's no background job continuously refreshing it. For genuinely continuous updates, the natural next step would be a scheduled trigger (e.g. a GitHub Action on a cron schedule running `run_pipeline.py` periodically and committing the output), which wasn't built here to keep the scope focused within the assignment window.
- **Every deal in the newsletter links back to its source article** and shows its publication date. The LLM never handles raw URLs directly — Google News redirect links are ~300-character opaque tokens, and asking an LLM to reproduce one verbatim proved fragile (it silently corrupted a link during testing). Instead, the LLM cites sources by a bracketed reference number (e.g. `[3]`), and a deterministic post-processing step (`resolve_citations()`) swaps each reference for the real markdown link pulled directly from the dataframe — the LLM never touches a URL at all.
